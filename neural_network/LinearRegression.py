...
def Lasso(x, y, alpha=0.1):
    ...
    #lasso.set_params(alpha=alpha) # set regularisation parameter
...
def calc_Variance(y, y_mu):
    n = np.size(y, 0)
    var_y = (1 / n) * sum((y - y_mu) ** 2)
    return var_y
...
def confidenceIntervall(y_true, y_predict, p, x_test, beta):
    N = np.size(y_true, 0)

    sigma2 = (1 / (N - (p - 1))) * (sum((y_true - y_predict) ** 2))
    # Betas varians:
    varBeta = np.linalg.pinv((x_test.T.dot(x_test))) * sigma2
...
def bootstrap(x, y, method, n_bootstrap=20):
    ...
    for i in range(n_bootstrap):
        ...
        if method == 'Ridge':
            ...
            #beta = Lasso(x_, y_.reshape(len(y_), 1), alpha=0.1)
            #print(np.shape(x_), np.shape(y_))
            #beta = beta1.reshape(len(beta1), 1)
...
#betaRidge, betaCoeffRidge, beta_confIntRidge = confidenceIntervall(y_test, energiesRidgepredicted, 1, X_test, betaRidge)
